...
/// Local stop words for CJK bigram filtering (extends shared list)
fn is_local_stop(word: &str) -> bool {
    if crate::stop_words::is_stop_word(word) {
        return true;
    }
    const EXTRA_STOPS: &[&str] = &[
        "的是", "在了", "不是", "就是", "也是", "都是",
        "为什么", "不知道", "没问题",
    ];
    EXTRA_STOPS.contains(&word)
}
